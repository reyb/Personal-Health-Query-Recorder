...
class NotificationEnd: WKInterfaceController {
    ...
    @IBAction func hourPickerChanged(_ value: Int) {
        ...
        defaults.set(hour, forKey: "hourEnd")
        resetTimeLabel()
    }
    
    @IBAction func minutePickerChanged(_ value: Int) {
        minute = minuteList[value].1
        defaults.set(minute, forKey: "minuteEnd")
        resetTimeLabel()
    }
    
    @IBAction func ampmPickerChanged(_ value: Int) {
        ampm = ampmList[value].1
        defaults.set(ampm, forKey: "ampmEnd")
        resetTimeLabel()
    }
    
    func resetTimeLabel () {
        let thisHour = defaults.string(forKey: "hourEnd") ?? ""
        let thisMinute = defaults.string(forKey: "minuteEnd") ?? ""
        let thisAMPM = defaults.string(forKey: "ampmEnd") ?? ""
        timeLabel.setText("Notifications will end at " + thisHour + ":" + thisMinute + " " + thisAMPM)
    }
}
